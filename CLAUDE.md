...
- **工具调用方式：使用 OpenAI function calling（`tools` 参数 + `tool_calls` 响应），而非让模型手写 XML/JSON 格式。推理模型（gpt-5.4、o1、o3）的 `content` 字段不可靠（经常为 null），但 `tool_calls` 始终正确返回。对于不支持 function calling 的模型（如 Grok-4 via realseek），自动降级为 prompt-based XML 格式（`<thought>` + `<action>`），通过 `ModelRoute.supportsFunctionCalling` 标志控制，`parseLegacyContent` 解析 XML 响应**
...
- **数据驱动的模型路由**：`lib/config.ts` 中的 `MODEL_ROUTES` 路由表将模型名前缀解析为 API 凭证（config 文件 + 环境变量名）和模型能力标志（`supportsFunctionCalling`）。Agent 的 `collectEnv()` 遍历路由表动态加载所有 provider 配置，Worker 的 `createProviderFromEnv()` 使用 `resolveModelRoute()` 创建对应的 `CompletionProvider`。添加新模型 provider 只需在路由表加一条记录，无需修改 Agent 代码
- **多模型工具调用适配**：支持两种工具调用模式，通过 `ModelRoute.supportsFunctionCalling` 标志自动切换。**Function calling 模式**（默认）：通过 OpenAI `tools` 参数 + `tool_calls` 响应，`CompletionResult.toolCalls` 是归一化抽象层。**XML prompt 模式**：对不支持 function calling 的模型（如 Grok-4 via realseek），系统提示中描述可用工具和 `<thought>` + `<action>` XML 格式，`parseLegacyContent` 解析响应，ContextManager 将 tool_call/tool 消息转为纯 assistant/user 消息。各厂商 function calling 格式不同（OpenAI: `tools` + `tool_calls`，Gemini: `functionDeclarations` + `functionCall`，Claude: `tools` + `tool_use`），未来接入非 OpenAI 兼容 API 时，在 `lib/adapters/` 中实现适配器，将厂商响应转换为 `CompletionResult` 即可，Planner 层无需修改
